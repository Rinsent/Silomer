...
void hit() {
  ...
  if (analogRead (piez_pin)  > stray + (stray * interferenceFilter / 100)) {
    ...
    disp = 0;
    //Устанавливаем последний МАКСИМАЛЬНЫЙ сигнал с пьезика на 0 (даем цифру обязательно!). Unsigned long потому что далее формулы работают со сбоями если стоит тип int. Сюда же вписываем нужное число (0-1023), если занимаемся отладкой.
    ...
    //ВНИМАНИЕ! Важный кусок автокалибровки максимальной силы удара. Если нанесен удар выше выставленной границы то граница поднимается до показателя силы удара
    if (calibrateHighInWork && exp_piez_max < piez_max) {
      exp_piez_max = piez_max;
    }
    //КОСТЫЛЬ! Я хз почему, но просто вставить число 100 не получается, надо обязательно присваивать ему тип! Короч, это увеличивает точность подсчетов.
    unsigned long k = 100;
    ...
    //А теперь убираем баг, когда при крайних к максимум значениях счет еще не дошел до 9999, но горит уже макс. лампочек
    ...
    do {
      if (millis() - tm > step_tm) {
        ...
        disp += disp_step;
        //Бегущие циферки примерно отображающие правильный счет
    ...
    while (true) {
      ...
      delay (50);
      //Эффекты в зависимости от настроек
      ...
      if (flagEf) {
        do {
          ...
          //Второй режим, внутри все так же как и в первом
            ...
            delay (blinkEfTm);
            //Прибавляем счетчик эффектов, если он превышает установленную в настройках величину, больше не мигаем
            counterEf++;
            //Если кол-во срабатывания эффектов в настройках 0, но они включены, работаем пока идет трек. Тут просто не поднимаем флаг отработки эффектов.
            if (counterEfMax > 0) {
              flagEf = false;
            }
          }
    ...
    displaySigns(0);
    //КОСТЫЛЬ!Вручную управляем CS дисплея, так как при работе через библиотеку в буфер лезет лишний кусок от мосфетов.
    digitalWrite (disp_pin, LOW);
    mosfet.digitalWrite (ALL, ALL, LOW);

    //Маленький доп.
    //Возможность переключить режим в случае циклического срабатывания удара
    //Отсечка таймера
    tm = millis();
    //В течении N времени
    do {
