...
//Функция калибровки нижних помех
void piezo_calibrateLow() {
  //Ставим показатель помех на 0
  stray = 0;
  ...
  //На всякий случай проверяем что бы помехи не вывалились за предполагаемую максимальную силу удара.
  if (exp_piez_max <= stray + (stray * interferenceFilter / 100) + numberOfLight) {
    ...
    //Если вылетели за пределы датчика, ставим макс. значение датчика
...
void piezo_calibrateHigh() {
  ...
  do {
    ...
    //Ожидаем удар
    while (true) {
      //Если пришел удар, и он преодолел фильтры
    ...
    //Если пьезик и так стоит на максимум, отключаем калибровку.
    if (exp_piez_max == 1023) {
      break;
    }
  } while (hit < 3);
...
void displaySigns(int val) {
  ...
  if (displayType == 0 ) {
    ...
    //Не забываем перевести cs пин в LOW, что бы не было проблем с отображением данных (конфликт с платами реле)
    digitalWrite (disp_pin, LOW);
  }
  //Если же стоит старый дисплей, или любой дисплей на каскаде сдвиговых регистров 74HC595.
    ...
    byte sign [4];
    //Переводим число в ячейки массива методом деления на порядок и нахождением остатка от 10. Обратите внимание, что тут они забиваются задом наперед. Тоесть число 1234 по факту превращается в число 4321. Связанно это с тем, что данные в дисплей тоже идут задом наперед, начиная с последней цифры.
    sign[3] = val / 1000 % 10;
    sign[2] = val / 100 % 10;
    sign[1] = val / 10 % 10;
    sign[0] = val % 10;
...
//Функция отображения уровня сложности
void difLvl () {
...
void clickSelect() {
  ...
  modeFlag = 0;
  //Сбрасывает таймер для корректного отображения символов на дисплее при переключении режима
  tm3 = millis();
}
...
void clickMinus() {
  switch (mode) {
    ...
    //В отладочном режиме запускает автокалибровку помех и сбрасываем флажок надписи, что бы после окончания калибровки опять были буквы
    case 1: modeFlag = 0; piezo_calibrateLow(); break;
    //В режиме помех, если значение помехи больше 0, уменьшает ее на 1. ВНИМАНИЕ! На дисплее в этот момент отображается значение ПОМЕХИ + ПРОЦЕНТНОГО ФИЛЬТРА! Отображаемый шаг может быть больше 1!
    case 2: if (stray > 0) {
        stray--;
    ...
    case 3: if (exp_piez_max > 0) {
        exp_piez_max--;
        //Если верхний предел подошел к помехе на помеху+процентный фильтр+ количество лампочек, не даем ему спуститься ниже во избежание циклического срабатывания hit()
        if (exp_piez_max <= stray + (stray * interferenceFilter / 100) + numberOfLight) {
          //Да, тут все верно считается! Напоминаю что Lbrd отображает значение УЖЕ С ФИЛЬТРОМ! Соответственно разница между Lbrd и Hbrd должна быть равна кол-ву лампочек + 1
          exp_piez_max = stray + (stray * interferenceFilter / 100) + numberOfLight + 1;
        }
        //Ставим таймер что бы пропустить заставку с буквами и сразу читать значение
        tm3 = millis() - 3001;
...
void longPressMinus() {
  ...
  switch (mode) {
    case 2: if (stray > 0) {
        stray--;
        //ПЛОХОЙ КОСТЫЛЬ!!! Позволяет не достигать крайних значений переменной за долю секунды, облегчает управление. Тут нужна паралельная работа, но на таймер почему то повесить не получается.
        delay(50);
      }
      break;
    case 3: if (exp_piez_max > 0) {
...
void longPressPlus () {
  ...
  switch (mode) {
    ...
    case 3: if (exp_piez_max < 1023) {
        exp_piez_max++;
        tm3 = millis() - 3001;
        //ПЛОХОЙ КОСТЫЛЬ!!! Позволяет не достигать крайних значений переменной за долю секунды, облегчает управление. Тут нужна параллельная работа, но на таймер почему то повесить не получается.
        delay(30);
      }
      break;
  }
...
void mode1() {
  ...
  //Если при переходе в цикл не показывалась надпись, то показать ее
  if (!modeFlag) {
  ...
  //Раз в N времени показываем максимальный результат за предыдущий цикл
...
//Функция помехи. Смотрим величину нижней помехи, на кнопках изменение ее величины
void mode2() {
  //Первые 3 секунды из 10 пишем слово заставку Lbrd (Low border)
  if (millis() - tm3 < 3000) {
    //Пишем библиотекой
    if (displayType == 0) {
      qd.displayDigits(QD_L, QD_b, QD_r, QD_d);
