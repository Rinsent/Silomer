...
//Порты
//Если код компилируют под Mega (ВНИМАНИЕ!!! Mega работает крайне нестабильно! Часто зависает! Рекомендуется использовать Uno/Nano)
...
int exp_piez_max = hitHight;        //Ожидаемый реальный максимальный сигнал с пьезика (мин. = 1, макс. = 1023). Зависит от клея на датчике, наличия и объема конденсатора, еще миллиона факторов. Устанавливается на основе тестов (отладка по пьезику). В глобалах потому что возможно будет динамическое изменение. На данный момент автокалибруется в случае превышения значения по максимальной силе удара.
volatile unsigned long tm = 0;      //Таймер 1 применяется для определения силы удара, работы с экраном, отображения уровня сложности
volatile unsigned long tm2 = 0;     //Таймер 2 применяется для работы с лампочками, отображения уровня сложности
volatile unsigned long tm3 = 0;     //Таймер 3 применяется для взаимодействия с функциями mode1-3
...
void setup() {
  ...
  if (chek) {
    ...
    //Поочередно включаем лампочки на шкале силы удара и параллельно просматриваем на дисплее все числа (без точек)
    //Отсечка для таймера
    tm = millis();
    ...
    while (true) {
      //Если лампочек больше 8 (кол-во выходов на модуле) Переходим на новый модуль и вычитаем кол-во уже зажженных лампочек из номера включаемого порта
      (lightNow < 8) ? mosfet.digitalWrite (0, lightNow, HIGH) : mosfet.digitalWrite (1, (lightNow - 8), HIGH);
      if (millis() - tm > 500) {
        //- 1 потому что кол-во лампочек считается от 1, а порты от 0
...
void loop() {
  ...
  buttonMinus.tick();
  buttonPlus.tick();
  //Действуем в соответствии с режимом, везде ставим функцию уровня сложности, в игровом (нулевом) режиме все время сбрасываем дисплей на 0.
  switch (mode) {
